...
cp -R -f $BLUEPRINTS_DIR/. ./deploy/blueprints || { echo "Failed to copy blueprints. Make sure you have contentful_aleph_blueprints checked out."; exit 1; }
...
yarn sentry-cli releases new contentful_aleph@$VERSION || { echo "Failed to create sentry release."; exit 1; }
yarn sentry-cli releases set-commits contentful_aleph@$VERSION --commit "$GITHUB_REPO@$VERSION" || { echo "Failed to associate commits with sentry release."; exit 1; }
